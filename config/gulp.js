 ...
 * Setup the env.
 */
const { isProd, isDev, isTest } = utils.detectEnv();
 ...
 */
const validate = () => {
	const src = utils.buildPath('*.html');
	const dest = utils.buildPath();

	return gulp
		.src(src)
		.pipe(gulpif(isTest, validator({
			'errors-only': true
		})))
		.pipe(gulp.dest(dest));
}
...
const optimize = () => {
	...
	const dest = utils.buildImagesPath();
	const imageMinSettings = isProd ? settings.imageminSettingsProduction : settings.imageminSettings;
...
gulp.task(
	'build',
	gulp.series(svgs, styles, scripts, pages, copy, validate, vendor)
);
